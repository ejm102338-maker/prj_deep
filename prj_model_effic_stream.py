...
logo_img = "./data/datasets_local/common_images/SceneStealer.png"
...
LABELS_P1 = ['문화재', '문화', '자연', '테마공원', '시장', '거리', '공원'] #df_info.columns[2:9].tolist()
LABELS_P2 = ['데이트/로맨틱', '힐링/여유', '액티브/아웃도어', '가족/키즈', '야경/밤감성'] #df_info.columns[9:14].tolist()
TARGET_COLUMNS = LABELS_P1 + LABELS_P2
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
IMG_DIR = "data/datasets_local/images"
MODEL_PATH_1 = "optuna_best_model_efficientnet_part1.pth"
MODEL_PATH_2 = "optuna_best_model_efficientnet_part2.pth"
# -------------------------------------------------
# GradCAM 클래스 정의
# -------------------------------------------------
class GradCAM:
...
def run_inference_with_cam(img_path, model1, model2):
    ...
    # Grad-CAM 엔진 초기화 (EfficientNet-B0의 마지막 Conv 레이어 타겟)
...
# 모델 적용
# -------------------------------------------------
m1 = load_model(MODEL_PATH_1, num_classes=len(LABELS_P1))
m2 = load_model(MODEL_PATH_2, num_classes=len(LABELS_P2))
...
# -------------------------------------------------
st.title("🎬 스크린 속 그곳으로")
st.subheader("캡쳐 한 장으로 떠나는 나만의 K‑로드 투어")

st.markdown("""
좋아하는 **K‑드라마 / 영화 장면**을 캡쳐해서 올리면  
...
with left:
    ...
    uploaded = st.file_uploader("📸 장면 캡쳐 업로드", type=["jpg","png","jpeg"])
    ...
    if uploaded:
        ...
        st.image(img, caption="업로드한 장면", use_container_width=True)
        ...
        for i, place in enumerate(places):
            ...
            with cols[i]:
                ...
                st.markdown(f"### {place['Place KorName']}")
                st.markdown(f"### {place['Place EngName']}")

                st.write(place["Place KorDesc"])
                st.write(place["Place EngDesc"])

                c1, c2 = st.columns(2)

                if c1.button("저장", key=f"save{i}"):

                    if place not in st.session_state.itinerary:
                        st.session_state.itinerary.append(place)
                        st.success("여정에 저장되었습니다")
...
with right:
    ...
    with st.expander("🧭 내 여행 리스트", expanded=True):
        if len(st.session_state.itinerary) == 0:
            st.info("아직 저장한 여행지가 없습니다.")
        else:
            for i, p in enumerate(st.session_state.itinerary):
                st.markdown(f"**{i+1}. {p['Place KorName']}**")
                st.caption(p['Place EngName'])
                
                if st.button("삭제", key=f"del{i}", use_container_width=True):
